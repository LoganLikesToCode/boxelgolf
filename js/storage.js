class Storage {
    ...
    getAll(filter = '') {
        ...
        for (var i = 0; i < localStorage.length; i++) {
            ...
            if (key.startsWith(filter)) data[key] = JSON.parse(value);
        }
        return data;
    }
                ...
                success: function(response) {
                    if (response.data) {
                        if (response.data.steamid) {
                            ...
                            app.ui.popup.add('<h2>Success!</h2><p>You can now play online with other players all across the world!</p>', [{ value: 'Continue' }]);
                        }
                        else {
                            app.ui.popup.add(response.data.message);
                        }
                    }
                }
            });
