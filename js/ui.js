class UI {
    ...
    finishCourse() {
        ...
        var record = app.score.saveScore(app.level.getLevelKey(), score);
        ...
        // Change outcome if disqualified
        if (score.time == 99.999) {
            text = '<h1>Oh no!</h1><h2>You ran out of time</h2>';
            reward = 0;
        }
        else {
        ...
        if (app.multiplayer.isConnected()) {
            ...
            options.push({ value: 'Continue', callback: next });
            ...
            if (app.multiplayer.isGuest()) {
                ...
                if (app.ui.view != 'level-editor') this.loadView('spectator');
    ...
    continue() {
        ...
        else if (this.view == 'course' || this.view == 'spectator') {
    ...
    showMultiplayerSettings() {
        ...
        if (app.multiplayer.isConnected() == false) {
            options.push(
                { value: 'Host', callback: function(e) {
    ...
    showUnverifiedMenu() {
        ...
        var options = [
            { label: 'View Store Page', value: 'Steam', class: 'action steam', style: 'margin-bottom: 1rem;', callback: function() { chrome.tabs.create({ url: 'https://store.steampowered.com/app/1945820/Boxel_Golf/' }); setTimeout(function() { _self.showMultiplayerSettings(); }, 250); }},
            { label: 'Step 1', value: 'Verify', style: 'margin-bottom: 1rem;', callback: function() { chrome.tabs.create({ url: 'https://www.dopplercreative.com/boxel-golf/verify/' }); setTimeout(function() { _self.showMultiplayerSettings(); }, 250); }},
            { label: 'Step 2', value: 'Token', callback: function() {
                    setTimeout(function() {
                        app.ui.popup.add('<h2>Please follow step 1 to generate a Steam token</h2>', [
